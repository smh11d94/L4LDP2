  ...
  <DragDropContext onDragEnd={onDragEnd}>
    <div className="flex justify-between items-center mb-4 w-full">
      <button 
        ...
        className="px-4 py-2 bg-emerald-200 rounded hover:bg-emerald-400 font-semibold"
      >
        Previous Month
      </button>
      <button 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
      {Object.entries(calendarData).map(([month, days]) => (
        <Card key={month} className="mb-6 w-full">
          ...
          <div className="grid grid-cols-7 gap-1 p-3">
            ...
            {Array.from({ length: days[0].date.getDay() }).map((_, index) => (
              <div key={`empty-${index}`} className="min-h-[100px] bg-gray-50 rounded" />
            ))}
            {days.map(daySlot => (
              <Droppable
              ...
              >
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`min-h-[100px] w-full p-2 rounded border ${
